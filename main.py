...
def main(autosubmit, source, values, url):
    ...
    tmp = list(filter(lambda i: i > 1000 and i <
               1000000000000000, list(map(int, tmp))))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simple script to generate a url for PVA attendance form")
    parser.add_argument("-s", "--source", action="store",
                        default="source.txt", help="source code for form webpage")
    parser.add_argument("-v", "--values", action="store",
                        default="values.json", help="values to insert into url")
    ...
    ).values, parser.parse_args().url, parser.parse_args().autosubmit
